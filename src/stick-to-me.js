  ...
  $.stickToMe = function (configs) {
    ...
    if (/Chrome/.test(navigator.userAgent)) {
      ...
      if (
        ...
        settings.disableleftscroll == true
      ) {
        chromealert = false;
      }
    }
    ...
    function ontheleave(e) {
      ...
      if (
        Math.round(e.pageX) == -1 ||
        Math.round(e.pageY) == -1 ||
        e.pageX == -3 ||
        e.pageY == -3
      ) {
      ...
      if (
        $.inArray(leaveside, settings.trigger) != -1 ||
        $.inArray("all", settings.trigger) != -1
      ) {
        var recenttime = new Date().getTime();
        if (recenttime - startuptime >= settings.mintime) {
          if (
            recenttime - startuptime <= settings.maxtime ||
            settings.maxtime == 0
          ) {
            if (howmanytimes < settings.maxamount || settings.maxamount == 0) {
              if (
                recenttime - lasttime >= settings.interval ||
                settings.interval == 0
              ) {
                if (chromealert) {
    ...
    function showbox() {
      if ($.data(document.body, "stick_var") != 1) {
        ...
        $('<div class="stick_block_layer"></div>')
          .appendTo("body")
